...
if (k1 != k2)   //Проверяю условие параллельности прямых (при равенстве "k1 = k2")
{
    double x = (b2 - b1) / (k1 - k2);   //Это уравнение вывел из двух исходных
    double y = k1 * x + b1;             //Находим "y", подставив "x" в первое уравнение (можно и во второе, без разницы)
    Console.WriteLine("Координаты точки пересечения прямых: x = {0}, y = {1}", x, y);
}
else
{
    if (b1 == b2) Console.WriteLine("Прямые совпадают!");   //Если "k1 = k2" и "b1 = b2", то прямые совпадают
    else Console.WriteLine("Прямые параллельны!");          //Если "k1 = k2", и "b" различны, то прямые параллельны
}